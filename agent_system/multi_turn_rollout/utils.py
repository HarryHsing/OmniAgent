...
# zhenghao add for dynamic batch size
'''
def adjust_dynamic_bsz_batch(data: DataProto, world_size: int) -> DataProto:
    bs = len(data)
    ...
    if remainder != 0:
        ...
        data = DataProto.concat([data, dup_proto])
    return data
'''
...
def adjust_batch(config, data: DataProto, mode="copy") -> DataProto:
    ...
    # print(f"size_divisor_ref     = {size_divisor_ref}")
    # print(f"size_divisor_rollout = {size_divisor_rollout}")
    # print(f"size_divisor_actor   = {size_divisor_actor}")
    # print(f"size_divisor (lcm)   = {size_divisor}")

    # check if the batch size is divisible by the dp size, if not, delete the last few samples to make it divisible
    bs = len(data)
    # print(f"adjust_batch len(data) = {bs}")
    remainder = bs % size_divisor
    # print(f"adjust_batch remainder = {remainder}")
    if remainder == 0:
        return data
    ...
    if mode == "delete":
        ...
        to_add      = size_divisor - remainder
        full_copies = to_add // bs          # How many full copies needed
        tail        = to_add %  bs          # How many remaining samples

        copies = [data] * full_copies
        if tail > 0:
            tail_idx = np.random.choice(bs, tail, replace=False)
            copies.append(data.select_idxs(tail_idx))

        adjusted_batch = DataProto.concat([data] + copies)
    else:
        raise ValueError(f"Unsupported mode: {mode}")

    # tmp_len = len(adjusted_batch)
    # print(f"adjust_batch done len(adjusted_batch) = {tmp_len}")
...
def filter_group_data(
    ...
    """
    Dynamic Sampling (adopted from DAPO):
      - Over-sample, then filter out groups where all episodes have identical rewards.
    This function also applies the same filtering to `exceeded` and returns it.
    """
    if last_try:          # Last round: no filtering
        return (batch_list, episode_rewards, episode_lengths,
                success, traj_uid, exceeded)

    batch_size = config.data.train_batch_size        # Original prompt count
    group_n    = config.env.rollout.n                # Repetitions per prompt
    if group_n <= 1:
        print("[filter_group_data] group_n <= 1, dynamic sampling disabled")
        return (batch_list, episode_rewards, episode_lengths,
                success, traj_uid, exceeded)

    keep_idx = []   # Sample indices to keep
    for i in range(batch_size):
        g_idx = np.arange(i * group_n, (i + 1) * group_n)
        g_rew = episode_rewards[g_idx]

        #  Sanity check: all UIDs within the same group must be identical
        for idx in g_idx:
            assert batch_list[idx][0]["uid"] == batch_list[g_idx[0]][0]["uid"]

        # If group rewards are not all identical -> keep entire group; otherwise discard
        if not np.all(g_rew == g_rew[0]):
            keep_idx.extend(g_idx.tolist())

    keep_idx = np.asarray(keep_idx, dtype=np.int64)

    if len(keep_idx) == 0:
        print(f"After filter has 0 keep_idx: {keep_idx}, skip filter for this batch step")
        return (batch_list, episode_rewards, episode_lengths,
                success, traj_uid, exceeded)

    # ---------- Execute filtering ----------
    batch_list       = [batch_list[i] for i in keep_idx]
    episode_rewards  = episode_rewards[keep_idx]
    episode_lengths  = episode_lengths[keep_idx]
    traj_uid         = traj_uid[keep_idx]
    exceeded         = exceeded[keep_idx]

    # Only filter success dict fields that are aligned with batch_list
    success = {
        k: (v[keep_idx] if len(v) == len(episode_rewards) else v)
