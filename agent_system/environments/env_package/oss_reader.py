...
class OssReader(metaclass=SingletonType):
    ...
    def read_audio_bin(self, oss_path: str, *, audio_start=0, audio_end=None, sr=16000):
        ...
        offset = int(audio_start * sr * 2)  # ensure even
        if offset % 2 != 0:
            offset += 1
    ...
    # @retry_on_error()
    # def get_video_frames_by_full_fps24_cache(
    #     self, oss_url: str, video_start=0, video_end=-1, n_frames=1, temporal_patch_size=2
    # ):
    #     """
    #     Inputs:
    #         oss_url: a video oss_url like `oss://bucket_name/video_path`
    #         video_start (float): video start second, inclusive
    #         video_end (float): video end second, exclusive, any non-positive value (<=0) means the end of full video
    #         n_frames (int or float):
    #             if > 0: n_frames (int)
    #             if < 0: -fps (float)
    #     Outputs:
    #         if no fps24 cache, return None
    #         else return a np.array with shape [n_frames, height, width, n_channels]
    #     """
    #
    #     def load_single_frame(offset_size):
    #         offset, size = offset_size
    #         try:
    #             data = self.read(f"{oss_url}.fps24.tar#{offset}-{offset + size}")
    #             frame = np.array(Image.open(BytesIO(data)).convert("RGB"))
    #             return frame
    #         except Exception as e:
    #             print(f"Error read {oss_url=} {offset=} {size=} {e=}")
    #             raise ValueError(f"Error read {oss_url=} {offset=} {size=} {e=}")
    #
    #     info = self.read(f"{oss_url}.fps24.txt").decode("utf-8")
    #     info = np.array([d.split() for d in info.splitlines()], dtype=np.int64)
    #
    #     # sanity check & build frame_indices
    #     video_start, video_end = video_start * 24, video_end * 24
    #     if video_end <= 0 or video_end >= len(info):
    #         video_end = len(info) - 1
    #     assert (
    #         video_start >= 0
    #         and video_start < len(info)
    #         and video_end >= 0
    #         and video_end <= len(info)
    #         and video_start < video_end
    #     ), f"{oss_url} {video_start} {video_end} {n_frames}"
    #     if n_frames < 0:
    #         fps = -n_frames
    #         n_frames = int((video_end - video_start) / 24 * fps)
    #     assert n_frames <= video_end - video_start, (
    #         f"{oss_url} {video_start} {video_end} {n_frames}"
    #     )
    #     idx = (
    #         np.linspace(video_start, video_end, n_frames, endpoint=True)
    #         .round()
    #         .astype(np.int64)
    #         .clip(0, len(info) - 1)
    #     )
    #     second_per_grid_t = (video_end - video_start) / (24 * (n_frames - 1) // temporal_patch_size)
    #     frame_indices = info[idx]
    #     assert len(frame_indices) == n_frames, (
    #         f"{oss_url} {video_start} {video_end} {n_frames}, {len(frame_indices)}"
    #     )
    #
    #     # load video frames
    #     frames = []
    #     max_workers = min(8, n_frames)
    #     with ThreadPoolExecutor(max_workers=max_workers) as executor:
    #         frames = list(executor.map(load_single_frame, frame_indices))
    #
    #     frames = np.stack(frames)
    #
    #     # build timestamps
    #     timestamps = [(frame_id - video_start) / 24 for frame_id in idx]
    #     assert len(timestamps) == n_frames, (
    #         f"{oss_url} {video_start} {video_end} {n_frames}, {len(timestamps)}"
    #     )
    #
    #     return frames, second_per_grid_t, timestamps
