...
class EnvironmentManagerBase:
    ...
    def success_evaluator(self, *args, **kwargs) -> Dict[str, np.ndarray]:
        """
        Extended success evaluator:
        - success_rate: per-sample success (original behavior)
        - success_overall: overall average success
        - success_<task>: per-task success rate
        - reward_<task>:  per-task reward mean
        """
        from collections import defaultdict
        import numpy as np
        ...
        # -------------------------------------
        # 1) Original logic: per-sample success_rate
        # -------------------------------------
        success = defaultdict(list)
        ...
        # success["success_rate"] should now be an array of length batch_size
        success_rate = np.array(success["success_rate"])
        assert len(success_rate) == batch_size

        # Add overall success rate (this is the "overall success" metric)
        success["success_overall"] = np.array([success_rate.mean()])

        # -------------------------------------
        # 2) New logic: per-task success / reward statistics
        # -------------------------------------
        task_success = defaultdict(list)
        task_reward  = defaultdict(list)

        for env_id in range(batch_size):
            last_info = total_infos[env_id][-1]

            qtype = last_info.get("question_type", "UNKNOWN")
            won   = 1 if last_info.get("won", False) else 0
            rew   = float(last_info.get("reward", 0.0))

            task_success[qtype].append(won)
            task_reward[qtype].append(rew)

        # Write per-task statistics into the success dictionary
        for qtype in task_success:
            success[f"success_{qtype}"] = np.array([np.mean(task_success[qtype])])
            success[f"reward_{qtype}"]  = np.array([np.mean(task_reward[qtype])])
