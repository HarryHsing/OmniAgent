...
MAX_PIXELS = _read_optional_int_env("OMNI_IMAGE_MAX_PIXELS", MAX_PIXELS)
VIDEO_MAX_PIXELS = _read_optional_int_env("OMNI_VIDEO_FRAME_MAX_PIXELS", VIDEO_MAX_PIXELS)
logger.info("set MAX_PIXELS: %s", MAX_PIXELS)
logger.info("set VIDEO_MAX_PIXELS: %s", VIDEO_MAX_PIXELS)
...
@retry()
def _download_image_http(url: str, timeout: float = 15) -> Image.Image:
    with requests.get(url, stream=True) as resp:
        resp.raise_for_status()
        with BytesIO(resp.content) as bio:
            return copy.deepcopy(Image.open(bio))
...
def fetch_image(ele: dict[str, str | Image.Image], size_factor: int = IMAGE_FACTOR) -> Image.Image:
    ...
    if isinstance(image, Image.Image):
        ...
        image_obj = _download_image_http(image)
...
def smart_nframes(
    ...
    if not (FRAME_FACTOR <= nframes and nframes <= total_frames):
        ...
        raise ValueError(
            f"{video_path} -> nframes should in interval "
            f"[{FRAME_FACTOR}, {total_frames}], but got {nframes}.")
    return nframes
...
def fetch_video(ele: dict, image_factor: int = IMAGE_FACTOR, return_video_sample_fps: bool = False) -> torch.Tensor | list[Image.Image]:
    if isinstance(ele["video"], str):
        ...
        try:
            ...
            if QUIET_LOGS:
                logger.info(f"video_reader_backend {video_reader_backend} error, fallback to torchvision, msg: {e}")
            else:
