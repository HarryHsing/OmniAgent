...
train_data_size=4
val_data_size=16                                                        #   val_data_size * N should be MICRO_RATIO times total GPU count
N=8                                                                     #   total_batch_size = train_data_size * N
...
export MICRO_RATIO=4                                                    # how many samples per gpu for vLLM                                                   
...
experiment_name=eval_${VAL_DATA_NAME}_${MODEL_NAME}_Step_${MIN_MAX_STEPS}-${max_steps}_K_${val_top_k}_P_${val_top_p}_Sample${val_do_sample}_DynStep${USE_DYNAMIC_STEP}_${MODE}
...
# ====== Head Node: Start Ray and Launch Evaluation ======
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    set +e 
    echo "[Head Node] Launching evaluation..."
