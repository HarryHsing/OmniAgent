...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        # zhenghao #############
        # 组装 vLLM 的 limit_mm_per_prompt 参数
        limit_mm_per_prompt = {}

        # 注意：要判断“是否存在且不是 None”，而不是用真值判断，从而允许 0
        if "limit_images" in config and config["limit_images"] is not None:
            limit_mm_per_prompt["image"] = int(config["limit_images"])

        if "limit_videos" in config and config["limit_videos"] is not None:
            limit_mm_per_prompt["video"] = int(config["limit_videos"])

        if "limit_audios" in config and config["limit_audios"] is not None:
            limit_mm_per_prompt["audio"] = int(config["limit_audios"])

        # 如果两个都没给，就传 None（维持 vLLM 默认行为）
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # print("vllm_rollout_spmd: do_sample, is_validate: ", do_sample, is_validate)
        if not do_sample:
            kwargs = {
                "best_of": 1,
                "top_p": 1.0,
        ...
        with self.update_sampling_params(**kwargs):
            ...
            try:
                ...
                for idx, inp in enumerate(vllm_inputs):
                    ...
                    # 假设 System Prompt 在第一个 <|im_end|> (151645) 之前
                    try:
                        ...
                        if sys_specials:
                            ...
                            print(f"[System Prompt Check] Clean. No multi-modal tokens found in first {first_im_end} tokens.")
                    except ValueError:
                        print("[System Prompt Check] Could not find <|im_end|>, prompt structure might be raw.")
