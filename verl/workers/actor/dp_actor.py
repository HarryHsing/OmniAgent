...
from verl.trainer.ppo.core_algos import (agg_loss, get_global_entropy_top_mask, get_policy_loss_fn,
                                         kl_penalty)
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(self, config, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        # print("DEBUG Zhenghao use_ulysses_sp and ulysses_sequence_parallel_size: ", self.use_ulysses_sp, self.ulysses_sequence_parallel_size)
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # Debug zhenghao 调试打印：当前序列长度以及多模态 token 尺寸
                # mm_shapes = {k: tuple(v.shape) for k, v in multi_modal_inputs.items()}
                # print("[DBG] rank =",torch.distributed.get_rank(), "  seq_len =", seqlen, "  mm_shapes =", mm_shapes, flush=True)
                # print("actor_module start (zhenghao)")
                ...
                # print("actor_module end (zhenghao)")
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        log_probs_lst = []
        entropy_lst = []
        # print(f"[rank {torch.distributed.get_rank()}] #micro_batches =", len(micro_batches), flush=True)
    ...
    def update_policy(self, data: DataProto):
        ...
        policy_loss_config = data.meta_info.get("policy_loss_config", False)
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    print(
                        f"mask{tuple(attention_mask.shape)} sum={attention_mask.sum().item()} | "
                        f"prompt{tuple(attention_mask[:, :attention_mask.shape[1]-response_length].shape)} "
                        f"sum={attention_mask[:, :attention_mask.shape[1]-response_length].sum().item()} | "
                        f"resp{tuple(attention_mask[:, -response_length:].shape)} "
                        f"sum={attention_mask[:, -response_length:].sum().item()}"
                    )
                    ...
                    if policy_loss_config:
                        self.config.policy_loss = policy_loss_config
                    ########
                    loss_mode = self.config.policy_loss.get("loss_mode", "vanilla")
                    # vanilla -> verl.trainer.ppo.core_algos.compute_policy_loss_vanilla
                    print(f"dp_actor loss_mode: {loss_mode}")
                    ...
                    if entropy_coeff != 0:
                        ...
                        micro_batch_metrics["actor/entropy_dp"] = entropy_loss.detach().item() * loss_scale_factor
                    else:
                        policy_loss = pg_loss
