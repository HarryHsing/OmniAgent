...
def compute_error_action_metrics(batch: DataProto):
    """
    统计 error_code / action_type 及各动作参数，
    对数值列自动忽略 NaN，若全 NaN 则不写该指标。
    """
    out   = {}
    total = len(batch)
    ...
    # ---------- error_code ----------
    if "error_code" in ntb:
        errs = ntb["error_code"]
        cnt  = Counter(errs.tolist())
        out.update({f"error/count/{k}": v for k, v in cnt.items()})
        out.update({f"error/{k}"      : v / total for k, v in cnt.items()})
        out["error/any_ratio"] = 1.0 - cnt.get("OK", 0) / total

    # ---------- action_type ----------
    if "action_type" in ntb:
        acts = ntb["action_type"]
        cnt_a = Counter(acts.tolist())
        out.update({f"action/count/{k}": v for k, v in cnt_a.items()})
        out.update({f"action/{k}"      : v / total for k, v in cnt_a.items()})

    # ---------- get_frames ----------
    if "n_frames" in ntb:
        vals = np.asarray(ntb["n_frames"], dtype=float)
        vals = vals[~np.isnan(vals)]
        if vals.size:
            out["get_frames/mean_n"] = float(vals.mean())
            out["get_frames/max_n"]  = float(vals.max())

    # ---------- clip / audio ----------
    for pref in ("clip", "audio"):
        dur_key = f"{pref}_dur"
        if dur_key in ntb:
            dur = np.asarray(ntb[dur_key], dtype=float)
            dur = dur[~np.isnan(dur)]
            if dur.size:
                out[f"{pref}/dur_mean"] = float(dur.mean())
                out[f"{pref}/dur_p95"]  = float(np.percentile(dur, 95))
                out[f"{pref}/dur_max"]  = float(dur.max())

        # 相对开始位置（只有 duration 字段有有效值且 video_duration 可用时）
        start_key = f"{pref}_start"
        if start_key in ntb and "video_duration" in ntb:
            starts = np.asarray(ntb[start_key], dtype=float)
            vdurs  = np.asarray(ntb["video_duration"], dtype=float)
            mask   = ~np.isnan(starts) & np.isfinite(starts) & (vdurs > 0)
            if mask.any():
                rel = starts[mask] / vdurs[mask]
                out[f"{pref}/start_mean_rel"] = float(rel.mean())
