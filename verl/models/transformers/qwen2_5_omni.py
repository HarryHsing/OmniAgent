...
from transformers.models.qwen2_5_omni.modeling_qwen2_5_omni import (
    Cache, Qwen2_5OmniForConditionalGeneration,
    Qwen2_5OmniThinkerCausalLMOutputWithPast,
    Qwen2_5OmniThinkerForConditionalGeneration, Qwen2_5OmniThinkerTextModel)
...
def get_rope_index(
    ...
    image_token_id = 151655 #processor.tokenizer.convert_tokens_to_ids(processor.image_token)
    video_token_id = 151656 #processor.tokenizer.convert_tokens_to_ids(processor.video_token)
    audio_token_id = 151646 #processor.tokenizer.convert_tokens_to_ids(processor.audio_token)
    vision_start_token_id = 151652 #processor.tokenizer.convert_tokens_to_ids(processor.vision_start_token)
    audio_start_token_id = 151647 #processor.tokenizer.convert_tokens_to_ids(processor.audio_start_token)
...
def qwen2_5_omni_verl_forward(
    ...
    if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
        ...
        # 为了支持batch 中存在有vl 和 无vl 时all_reduce
        if pixel_values is None:
            print("no image, add a fake image to all_reduce")
            ...
            grid_thw = torch.tensor([[1,2,2]], dtype=torch.long, device=inputs_embeds.device)
            image_embeds = self.visual(pixel_values, grid_thw=grid_thw)
            inputs_embeds = inputs_embeds + image_embeds.mean() * 0
        if pixel_values_videos is None:
            print("no video, add a fake video to all_reduce")
            pe  = self.visual.patch_embed
            vis = self.visual
            C   = pe.in_channels                # =3
            P   = pe.patch_size                 # =14
            T0  = pe.temporal_patch_size        # =2
            S   = vis.spatial_merge_size        # =2
            expect_patch_dim  = C * T0 * P * P  # =1176
            expect_num_tokens = T0 * S * S      # =8
            pixel_values_videos = torch.zeros(
                (expect_num_tokens, expect_patch_dim),
                dtype=inputs_embeds.dtype,
                device=inputs_embeds.device,
            )
            grid_thw_vid = torch.tensor([[T0, S, S]], dtype=torch.long,
                                        device=inputs_embeds.device)
            # print('pixel_values_videos shape: ', pixel_values_videos.shape)
            # print('grid_thw_vid shape: ', grid_thw_vid.shape)
            video_embeds = self.visual(pixel_values_videos, grid_thw=grid_thw_vid)
            inputs_embeds = inputs_embeds + video_embeds.mean().to(inputs_embeds.dtype) * 0
        # 为了支持batch 中存在有audio 和 无audio 时all_reduce
        if input_features is None:
            print("no audio, add a fake audio to all_reduce")
            audio_feature_lengths = torch.tensor([5], dtype=torch.long, device=inputs_embeds.device)
            input_features = torch.zeros(128, 5).to(dtype=inputs_embeds.dtype, device=inputs_embeds.device)
...
BASE_THINKER_FORWARD = qwen2_5_omni_verl_forward
