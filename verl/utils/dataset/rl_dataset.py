...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        for data_file in self.data_files:
            # 根据后缀决定使用 json 还是 parquet loader
            lower = data_file.lower()
            if lower.endswith((".jsonl", ".json")):
                dataset = datasets.load_dataset(
                    "json", data_files=data_file, split="train",  # or streaming=True
                )
            else:   # 默认仍按 parquet 读取
                dataset = datasets.load_dataset(
                    "parquet", data_files=data_file, split="train"
                )
    ...
    def __getitem__(self, item):
        # 1. 必须深拷贝！这是解决“跑100个batch报错”的关键。
        # 不拷贝的话，Dataset 内部的缓存会一直持有每一个 row_dict。
        row_dict_raw = self.dataframe[item]
        row_dict = copy.deepcopy(row_dict_raw)
        
        # 2. 占位符逻辑：给 1 个长度的假数据，反正你会重新计算。
        # 这解决了 AttributeError，且内存占用极小。
        dummy_tensor = torch.zeros(1, dtype=torch.long)
        row_dict["input_ids"] = dummy_tensor
        row_dict["attention_mask"] = dummy_tensor
        row_dict["position_ids"] = dummy_tensor
        row_dict["raw_prompt_ids"] = [0] # List 格式
        
        # 3. 此时 row_dict 包含了你真正需要的：
        # video, question_type, question, answer, options, fps, duration_seconds, has_audio
        # 这些数据会在 RayPPOTrainer 里的 pop 中被存入 gen_batch。
        
        # 4. 彻底清理 Dataset 阶段可能产生的任何中间大负载
        row_dict.pop("multi_modal_inputs", None)
        row_dict.pop("multi_modal_data", None)
    ...
    #     # --- [1.1 彻底切断与 Dataset 内部缓存的联系] ---
    #     # datasets 库有时会缓存返回的对象，deepcopy 保证 row_dict 是完全独立的
    #     row_dict_raw = self.dataframe[item]
    #     row_dict = copy.deepcopy(row_dict_raw)
    ...
    #         # zhenghao omni add, if raw_prompt is a list, raw_prompt = raw_prompt[0]
    #         raw_prompt = raw_prompt[0] if isinstance(raw_prompt, list) else raw_prompt
    ...
    #         # print("Zhenghao: ", type(raw_prompt), raw_prompt)
