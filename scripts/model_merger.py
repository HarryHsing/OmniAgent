...
python scripts/model_merger.py merge   \
    --backend fsdp \
    --local_dir /path/to/checkpoints/global_step_50/actor \
    --hf_model_path /path/to/base_hf_model \
    --target_dir /path/to/merged_hf_model
...
# Only register the model, no longer register config (config is already in the library)
if Qwen2_5OmniConfig not in MODEL_FOR_CAUSAL_LM_MAPPING:
    MODEL_FOR_CAUSAL_LM_MAPPING.register(
        Qwen2_5OmniConfig, Qwen2_5OmniForConditionalGeneration
    )
if Qwen2_5OmniConfig not in MODEL_FOR_VISION_2_SEQ_MAPPING:
    MODEL_FOR_VISION_2_SEQ_MAPPING.register(
        Qwen2_5OmniConfig, Qwen2_5OmniForConditionalGeneration
    )
from transformers.models.auto.modeling_auto import MODEL_FOR_CAUSAL_LM_MAPPING
print("Qwen2_5Omni in causal mapping? ", Qwen2_5OmniConfig in MODEL_FOR_CAUSAL_LM_MAPPING)
